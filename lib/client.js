window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function buildSideChatPrompt(input) {
			...
			if (input.transcript.length > 0) {
				const lines = input.transcript.slice(-4).map((entry) => `[${entry.role}]: ${clip(entry.text, ENTRY_MAX_CHARS)}`);
		...
		*/
		function sideRowsOf(snapshot, firstQuestion) {
			const rows = [];
			...
			for (const key of snapshot.chat.order) {
				...
				if (node.kind === "user" || node.kind === "steering") {
					...
					const shown = sawUserRow || firstQuestion === void 0 ? text : firstQuestion;
					sawUserRow = true;
					if (text !== "") rows.push({
						role: "user",
						kind: "text",
					...
					for (let i = 0; i < blocks.length; i++) {
						...
						if (block.kind === "reasoning") {
							...
							rows.push({
								...
								kind: "text",
								text,
								state: running ? "streaming" : "final"
							});
							emitted = true;
						}
					}
					if (!emitted && running) rows.push({
						role: "assistant",
						kind: "text",
						text: "",
						state: "streaming"
		...
		*/
		function deriveSideView(snapshot, quote, state) {
			...
			return {
				...
				rows: snapshot === void 0 ? [] : sideRowsOf(snapshot, state.firstQuestion)
			};
		}
		//#endregion
		//#region src/client/controller.ts
		/** Create the minimal synchronous snapshot source required by the panel hook. */
		function createMutableSnapshot(initial) {
		...
		var SideChatController = class {
			...
			*/
			...
			#recordOf(sessionId) {
				...
				if (record === void 0) {
					record = {
						store: createMutableSnapshot(deriveSideView(void 0, "", { status: "idle" })),
						quote: {
							nodeKey: "",
							text: "",
							rect: {
		...
		*/
		...
		//#endregion
		//#region \0side-chat-css:src/client/SelectionLauncher.module.css.mjs
		const css$1 = ".It3RUq_layer{pointer-events:none;position:absolute;inset:0}.It3RUq_ask{border:1px solid var(--dsw-alias-border-l2);box-sizing:border-box;background:var(--dsw-alias-button-elevated-fill);max-width:200px;color:var(--dsw-alias-label-primary);cursor:pointer;pointer-events:auto;border-radius:999px;align-items:center;gap:6px;padding:5px 10px;font-family:inherit;font-size:12px;font-weight:500;line-height:18px;display:inline-flex;position:absolute;box-shadow:0 4px 12px #0000001f}.It3RUq_ask:hover{background:var(--dsw-alias-button-floating-hover)}.It3RUq_ask:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.It3RUq_askLabel{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}";
		...
		*/
		...
		//#endregion
		//#region \0side-chat-css:src/client/SideChatPanel.module.css.mjs
		const css = ".Wborea_root{border-left:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);flex-direction:column;min-width:0;height:100%;display:flex}.Wborea_header{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:8px;padding:14px 12px 12px;display:flex}.Wborea_title{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:14px;font-weight:500;line-height:20px;overflow:hidden}.Wborea_close{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:999px;flex:none;place-items:center;display:grid}.Wborea_close:hover{background:var(--dsw-alias-interactive-bg-hover)}.Wborea_close:focus-visible,.Wborea_primary:focus-visible,.Wborea_secondary:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.Wborea_quote{border-left:3px solid var(--dsw-alias-border-l4);background:var(--dsw-alias-interactive-bg-hover);margin:0;padding:10px 12px}.Wborea_quoteLabel{color:var(--dsw-alias-label-tertiary);margin-bottom:4px;font-size:11px;font-weight:500;line-height:16px}.Wborea_quoteText{color:var(--dsw-alias-label-secondary);word-break:break-word;-webkit-line-clamp:4;-webkit-box-orient:vertical;font-size:12px;line-height:19px;display:-webkit-box;overflow:hidden}.Wborea_body{flex-direction:column;flex:1;gap:16px;min-height:0;padding:16px;display:flex;overflow-y:auto}.Wborea_empty{color:var(--dsw-alias-label-tertiary);padding:2px 0;font-size:14px;line-height:24px}.Wborea_row{flex-direction:column;min-width:0;display:flex}.Wborea_row[data-role=user]{align-items:flex-end}.Wborea_bubbleUser{background:var(--dsw-specific-bubble);max-width:min(525px,82%);color:var(--dsw-alias-label-primary);word-break:break-word;white-space:pre-wrap;border-radius:22px;padding:10px 16px;font-size:16px;line-height:24px}.Wborea_assistantText{color:var(--dsw-alias-label-primary);word-break:break-word;white-space:pre-wrap;font-size:16px;line-height:28px}.Wborea_streaming{margin-left:1px;animation:1s steps(2,start) infinite Wborea_side-chat-blink}@keyframes Wborea_side-chat-blink{to{visibility:hidden}}.Wborea_thinkRoot{flex-direction:column;display:flex}.Wborea_thinkRoot .Wborea_thinkRow{position:relative;overflow:hidden}.Wborea_thinkRoot[data-state=running] .Wborea_thinkRow:after{content:\"\";inset-block:0;background:linear-gradient(90deg, transparent 0%, color-mix(in srgb, var(--dsw-alias-bg-base) 60%, transparent) 55%, transparent 100%);pointer-events:none;width:300px;animation:2.6s ease-out infinite Wborea_side-chat-think-sweep;position:absolute;left:0}@keyframes Wborea_side-chat-think-sweep{0%{left:-300px}90%,to{left:100%}}.Wborea_thinkLeading{flex-shrink:0}.Wborea_thinkChevron{color:var(--dsw-alias-label-secondary)}.Wborea_thinkTitle{font-weight:400}.Wborea_thinkSeparator{background:var(--dsw-alias-label-caption);border-radius:1px;flex:none;width:2px;height:2px;margin:0 8px}.Wborea_thinkSummary{min-width:0;color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:auto;font-size:14px;line-height:24px;overflow:hidden}.Wborea_thinkSummary[data-follow-end]{text-overflow:clip}.Wborea_thinkBody{color:var(--dsw-alias-label-tertiary);white-space:pre-wrap;word-break:break-word;padding:4px 0 4px 22px;font-size:14px;line-height:24px}@media (prefers-reduced-motion:reduce){.Wborea_streaming,.Wborea_thinkRoot[data-state=running] .Wborea_thinkRow:after{animation:none}}.Wborea_error{background:var(--dsw-alias-interactive-bg-hover-danger);color:var(--dsw-alias-label-primary);word-break:break-word;border-radius:8px;margin-top:4px;padding:8px 10px;font-size:12px;line-height:18px}.Wborea_composer{border-top:1px solid var(--dsw-alias-border-l2);flex:none;padding:10px 12px 12px}.Wborea_input{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l3);background:var(--dsw-alias-bg-layer-1);width:100%;color:var(--dsw-alias-label-primary);resize:vertical;border-radius:10px;padding:8px 10px;font-family:inherit;font-size:13px;line-height:20px}.Wborea_input::placeholder{color:var(--dsw-alias-label-caption);-webkit-text-fill-color:var(--dsw-alias-label-caption)}.Wborea_input:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.Wborea_input:disabled{opacity:.6}.Wborea_controls{justify-content:flex-end;gap:8px;margin-top:8px;display:flex}.Wborea_primary,.Wborea_secondary{cursor:pointer;border-radius:999px;padding:5px 14px;font-family:inherit;font-size:12px;font-weight:500;line-height:18px}.Wborea_primary{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);border:none}.Wborea_primary:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}.Wborea_primary:disabled{background:var(--dsw-alias-button-primary-dimmed);cursor:default}.Wborea_secondary{border:1px solid var(--dsw-alias-border-l3);color:var(--dsw-alias-label-secondary);background:0 0}.Wborea_secondary:hover{background:var(--dsw-alias-interactive-bg-hover)}";
		...
		var SideChatPanel_module_css_default = {
			"assistantText": "Wborea_assistantText",
			"body": "Wborea_body",
			"bubbleUser": "Wborea_bubbleUser",
			"close": "Wborea_close",
			"composer": "Wborea_composer",
			"controls": "Wborea_controls",
			"empty": "Wborea_empty",
			"error": "Wborea_error",
			"header": "Wborea_header",
			"input": "Wborea_input",
			"primary": "Wborea_primary",
			"quote": "Wborea_quote",
			"quoteLabel": "Wborea_quoteLabel",
			"quoteText": "Wborea_quoteText",
			"root": "Wborea_root",
			"row": "Wborea_row",
			"secondary": "Wborea_secondary",
			"side-chat-blink": "Wborea_side-chat-blink",
			"side-chat-think-sweep": "Wborea_side-chat-think-sweep",
			"streaming": "Wborea_streaming",
			"thinkBody": "Wborea_thinkBody",
			"thinkChevron": "Wborea_thinkChevron",
			"thinkLeading": "Wborea_thinkLeading",
			"thinkRoot": "Wborea_thinkRoot",
			"thinkRow": "Wborea_thinkRow",
			"thinkSeparator": "Wborea_thinkSeparator",
			"thinkSummary": "Wborea_thinkSummary",
			"thinkTitle": "Wborea_thinkTitle",
			"title": "Wborea_title"
		};
		//#endregion
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							view.rows.map((row, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: row.kind === "reasoning" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ThinkRow, {
									text: row.text,
									running: row.state === "streaming"
								}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: row.role === "user" ? SideChatPanel_module_css_default.bubbleUser : SideChatPanel_module_css_default.assistantText,
							...
							view.running && view.rows[view.rows.length - 1]?.state !== "streaming" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: SideChatPanel_module_css_default.assistantText,
		...
		*/
		function ThinkRow({ text, running }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: SideChatPanel_module_css_default.thinkRoot,
				"data-state": running ? "running" : "ok",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.DisclosureRow, {
					rowClassName: SideChatPanel_module_css_default.thinkRow,
					leadingClassName: SideChatPanel_module_css_default.thinkLeading,
					titleClassName: SideChatPanel_module_css_default.thinkTitle,
					chevronClassName: SideChatPanel_module_css_default.thinkChevron,
					...
					collapsedContent: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: SideChatPanel_module_css_default.thinkSeparator,
						"aria-hidden": true
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						ref: summaryRef,
						className: SideChatPanel_module_css_default.thinkSummary,
						"data-follow-end": running || void 0,
						children: summary
					})] }),
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: SideChatPanel_module_css_default.thinkBody,
						children: text
					})
				})
			});
